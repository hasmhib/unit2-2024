...
## Test Plan 
| Task no | Test type | Procedure | Expected Outcome |
|-----------|--------|-----------|------------------|
|1|Testing the arduino code for three sensors|Upload the code to the arduino, then use the function read_arduino to collect the data.|The data including humidity and temperature from each sensor are shown when I run the code every 5 min.|
|2|Testing the function to save data to csv file|Run the function called save_csv.|The data of humidity and temperature from each sensor is collected in the csv file.|
|3|Testing the function to send the data to api server|Run the function called send_data.Check if data is sent to the api server by looking at the adequate sensor id and see if it matches the id of sensors used for the data collecting.|The data is sent to the server with accurate id.|
|4|Testing the code for graph of the data from remote sensors|Run the code both for humidity and temperature.|Four graphs which represent analysis and the value from each sensors are shown.The biggest graph is for analysis and this graph has three lines for mean and midian, maximum and minimum.|
|5|Testing the code for graph of the data from local sensors.|Run the code both for humidity and temperature.|Three lines which are for each sensors are shown in one huge graph. |
